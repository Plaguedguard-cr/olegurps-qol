...
export async function openAmmoManager() {
  ...
  function getMaximumShots(attack, loaded) {
    return fireService.getMaximumShots(attack, loaded);
  }
  ...
  function calculateEffectiveFireSkill(attack, values, rangeBands, targetingService) {
    ...
    const settings = getTaggedModifierSettings();
    const effectRangePenalty = settings?.autoAdd
    ...
    const aimBonus = fireService.calculateAimBonus(attack?.acc, values?.aimSeconds);
    const bracingBonus = fireService.calculateBracingBonus(values?.braced, values?.aimSeconds);
    const laserBonus = parseBoolean(values?.laserSight) ? 1 : 0;
    const fireMode = getFireModeState(attack, values, rangeBands);
    const rapidFireBonus = calculateRapidFireBonus(fireMode.effectiveRoF);
  ...
  function parseShotOptions(weapon, attack, values, rangeBands, targetingService, effectRangePenalty = null) {
    ...
    const maximumShots = getMaximumShots(attack, loaded);
    ...
    const aimBonus = fireService.calculateAimBonus(attack?.acc, aimSeconds);
    const braced = parseBoolean(values.braced);
    const bracingBonus = fireService.calculateBracingBonus(braced, aimSeconds);
    ...
    if (shotsText === "" || !Number.isInteger(shots) || shots < 1 || shots > maximumShots) {
      errors.push(`количество выстрелов должно быть целым числом от 1 до ${maximumShots}`);
    }
  ...
  async function openFirePreparation(state, weaponId, managerApp) {
    ...
    const preparation = new FirePreparationApp({
      ...
      calculateAimBonus: aimSeconds => fireService.calculateAimBonus(attack.acc, aimSeconds),
      calculateBracingBonus: (braced, aimSeconds) => fireService.calculateBracingBonus(braced, aimSeconds),
